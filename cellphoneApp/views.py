...
def login_view(request):
    if request.method == 'POST':
        ...
        password_user_input = data.get('password')
        # hashed_password = bcrypt.hashpw(password_user_input.encode('utf-8'), salt)
        try:
        ...
        print(user)
        if check_password(password_user_input, user.passWord):
            # Nếu mật khẩu đúng, đăng nhập thành công
            print('no co chay vo so sanh')
            user_dict = {
                'id': user.Id,
                'name': user.Name,
                'email': user.Email,
                ...
                'role_id': user.idRole_id,
            }

            user_json = json.dumps(user_dict, ensure_ascii=False)

            return JsonResponse({'status': 'ok','user' : user_json })
        else:
            return JsonResponse({'status': 'fail', 'message': 'Invalid login'})
    else:
